...
function getChangedFiles(projectRoot) {
    try {
        const staged = (0, child_process_1.execSync)('git diff --name-only HEAD', {
            cwd: projectRoot,
            encoding: 'utf-8'
        }).trim();
...
function checkCompleteness(dependencyMap, projectRoot) {
...
function formatDependencyMap(map) {
    ...
    const lines = [
        ...
        '',
        ...map.nodes.map(node => `${typeEmoji[node.type] || '•'} ${node.file}\n   └─ ${node.reason}`),
        '',
        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
